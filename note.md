...
-   The analysis is based on the available Superstore dataset and does
    not include additional operational cost data.
-   Relationships between discounts and profit are descriptive and do
    not prove causation.
-   Duplicate order-line combinations were detected but not removed
    because their validity was not independently established.
-   The dataset does not provide enough information to determine all
    underlying reasons for negative profitability.
-   Historical customer lifetime profit should not be interpreted as a
    prediction of future customer value.
...
Overall, the project demonstrates how SQL can transform transactional
data into structured business analysis through aggregation, conditional
logic, date calculations, window functions, and multi-dimensional
analysis.
